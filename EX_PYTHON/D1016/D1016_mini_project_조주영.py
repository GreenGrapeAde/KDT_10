...
def update_listbox():
    ...
    for sub in subscriptions:
        listbox.insert(tk.END, sub['name'])
...
def show_info(event):
    ...
    next_pay_day = min(int(sub['date']), next_last_day)
...
def add_subscription():
    ...
    tk.Label(add_win, text="앱 이름").pack(pady=3)
    entry_name = tk.Entry(add_win)
    entry_name.pack()

    tk.Label(add_win, text="이번달 결제(예정)일").pack(pady=3)
    entry_date = tk.Entry(add_win)
    entry_date.pack()

    tk.Label(add_win, text="가격").pack(pady=3)
    entry_price = tk.Entry(add_win)
    entry_price.pack()

    tk.Button(add_win, text="확인", command=confirm_add).pack(pady=10)
...
def delete_subscription():
    ...
    tk.Label(del_win, text="삭제할 앱 이름").pack(pady=5)
    entry_del = tk.Entry(del_win)
    entry_del.pack()

    tk.Button(del_win, text="삭제", command=confirm_delete).pack(pady=10)
...
def edit_subscription():
    ...
    def confirm_edit():
        ...
        for sub in subscriptions:
            if sub['name'] == name:
                ...
                subscriptions.sort(key=lambda sub: int(sub['date']))
                update_listbox()
                update_total() 
    ...
    tk.Label(edit_win, text="변경할 앱 이름").pack(pady=3)
    entry_name = tk.Entry(edit_win)
    entry_name.pack()

    tk.Label(edit_win, text="새 결제(예정)일").pack(pady=3)
    entry_date = tk.Entry(edit_win)
    entry_date.pack()

    tk.Label(edit_win, text="새 가격").pack(pady=3)
    entry_price = tk.Entry(edit_win)
    entry_price.pack()

    tk.Button(edit_win, text="확인", command=confirm_edit).pack(pady=10)

def go_home():
    """홈 버튼 클릭 시 초기 화면 상태로 복귀"""
    listbox.selection_clear(0, tk.END)         # 리스트 선택 해제
...
btn_home = tk.Button(top_frame, text="홈", width=10, command=go_home)
btn_add = tk.Button(top_frame, text="추가", width=10, command=add_subscription)
btn_delete = tk.Button(top_frame, text="삭제", width=10, command=delete_subscription)
btn_edit = tk.Button(top_frame, text="변경", width=10, command=edit_subscription)
...
tk.Label(list_frame, text="정기결제 목록").pack()
listbox = tk.Listbox(list_frame, width=30, height=15)
listbox.pack(side='left', fill='y')
listbox.bind("<<ListboxSelect>>", show_info)

# 오른쪽: 정보 표시 라벨
info_frame = tk.Frame(bottom_frame)
info_frame.pack(side='top', fill='both', expand=True, padx=20, pady=(20, 100))

tk.Label(info_frame, text="결제 정보", font=('Arial', 14, 'bold')).pack(anchor='w')
info_text = tk.StringVar()
infoLB = tk.Label(info_frame, textvariable=info_text, justify='left', font=('Arial', 12))
...
total_label = tk.Label(total_frame, text=f"이번달 총 결제 금액: 0원", font=('Arial', 12, 'bold'))
