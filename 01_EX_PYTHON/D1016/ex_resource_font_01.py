...
root.geometry("800x300+500+300")           ## 뒤에 +는 모니터 상에 띄우는 위치
...
previewLB.pack(side='left', padx=10, pady=10)  ## 리스트 박스에 padx=10 있기 때문에 여백 생김
...
## ---------------------------------------------------------------------
def show_font(event):
    ## 리스트박스에서 마우스로 선택한 것 가져오기
    seletFont = fontLBX.get(fontLBX.curselection())

    ## 선택한 폰트를 미리보기 레이블에 적용 설정
    ## Widget인스턴스.config(옵션/속성명-새로운 값)
...
##  << >> → Tkinter가 미리 정의한 “가상 이벤트 이름(Virtual Event Name)”
